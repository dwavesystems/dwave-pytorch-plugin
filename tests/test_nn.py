...
from dwave.plugins.torch.nn import GivensRotationLayer, LinearBlock, SkipLinear, store_config
from dwave.plugins.torch.nn.modules.orthogonal import _get_blocks_edges
    ...
    def test_bad_initialization_parameters(self):
        with self.subTest("n less than 2"):
            with self.assertRaises(ValueError):
                GivensRotationLayer(1)
            with self.assertRaises(ValueError):
                GivensRotationLayer(0)
            with self.assertRaises(ValueError):
                GivensRotationLayer(-5)
        with self.subTest("n not integer"):
            with self.assertRaises(ValueError):
                GivensRotationLayer(3.5)
            with self.assertRaises(ValueError):
                GivensRotationLayer("a string")
        with self.subTest("bias not boolean"):
            with self.assertRaises(ValueError):
                GivensRotationLayer(5, bias="another string")
            with self.assertRaises(ValueError):
                GivensRotationLayer(5, bias=1)

    @parameterized.expand([9, 10])
    def test_get_blocks_edges(self, n):
        blocks = _get_blocks_edges(n)
